...
public class ToggleWeapon extends NamedWeapon{
    ...
    /** Weapon visibility specification. */
    ...
    public void unlock(){
        Core.settings.put("weapon-" + name + "-unlocked", true);
    }

    public boolean isUnlocked(){
        if(!requiresUnlock) return true;
        return SafeSettings.getBool("weapon-" + name + "-unlocked", false);
    }

    private boolean isEnabled(ToggleWeaponMount t){
        return t.enabled && visibility.visible();
    }
    ...
    public void drawOutline(Unit unit, WeaponMount mount){
        if(mount instanceof ToggleWeaponMount t){
            if(isEnabled(t)) super.drawOutline(unit, mount);
            return;
        }
    ...
    public void draw(Unit unit, WeaponMount mount){
        if(mount instanceof ToggleWeaponMount t){
            if(isEnabled(t)) super.draw(unit, mount);
            return;
        }
    ...
    protected float bulletRotation(Unit unit, WeaponMount mount, float bulletX, float bulletY){
        if(mount instanceof ToggleWeaponMount t){
            if(isEnabled(t)) return super.bulletRotation(unit, mount, bulletX, bulletY);
        }
    ...
    protected void shoot(Unit unit, WeaponMount mount, float shootX, float shootY, float rotation){
        if(mount instanceof ToggleWeaponMount t){
            if(isEnabled(t)) super.shoot(unit, mount, shootX, shootY, rotation);
            return;
        }
    ...
    protected void bullet(Unit unit, WeaponMount mount, float xOffset, float yOffset, float angleOffset, Mover mover){
        if(mount instanceof ToggleWeaponMount t){
            if(isEnabled(t)) super.bullet(unit, mount, xOffset, yOffset, angleOffset, mover);
            return;
        }
    ...
    protected void handleBullet(Unit unit, WeaponMount mount, Bullet bullet){
        if(mount instanceof ToggleWeaponMount t){
            if(isEnabled(t)) super.handleBullet(unit, mount, bullet);
            return;
        }
    ...
    public static class WeaponVisibility{
        public static final WeaponVisibility
                hidden = new WeaponVisibility(() -> false),
                shown = new WeaponVisibility(() -> true),
                sandboxOnly = new WeaponVisibility(() -> Vars.state == null || Vars.state.rules.infiniteResources),
                lightingOnly = new WeaponVisibility(() -> Vars.state == null || Vars.state.rules.lighting);
        ...
        public WeaponVisibility(Boolp visible){
            this.visible = () -> visible.get() || anyAll();
        }

        public static boolean anyAll(){
            return SafeSettings.getBool("yellow-all-weapons-anywhere", false);
        }
    }
}
